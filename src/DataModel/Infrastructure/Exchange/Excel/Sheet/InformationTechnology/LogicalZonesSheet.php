...
class LogicalZonesSheet extends AbstractSwarkExcelSheet implements FromGenerator, WithTitle, SkipsEmptyRows
{
    ...
    {
        $this->allowRelationshipType = RelationshipType::upsert('allow_access',
            ['name' => 'Allow access', 'source_name' => 'Source', 'target_name' => 'Target']);
        $this->denyRelationshipType = RelationshipType::upsert('deny_access',
    ...
    protected function importRow(RowContext $row)
    {
        ...
        $this->compositeKeyContainer->set('logical_zone', $logicalZone->configurationItem->scomp_id, $logicalZone->id);
        $this->compositeKeyContainer->set('logical_zone', Str::upper($logicalZone->configurationItem->scomp_id), $logicalZone->id);
        ...
        foreach ($this->processedRows as $row) {
            ...
            foreach ($allowTargetZoneIds as $allowTargetZoneId) {
                Relationship::updateOrCreate([
                    'source_type' => 'logical_zone',
                    'source_id' => $sourceZoneId,
                    'target_type' => 'logical_zone',
                    'target_id' => $allowTargetZoneId,
