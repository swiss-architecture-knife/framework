...
class BaremetalsSheet extends AbstractSwarkExcelSheet implements FromGenerator, WithTitle, SkipsEmptyRows
{
    ...
    protected function importRow(RowContext $row)
    {
        ...
        $this->compositeKeyContainer->set('baremetal', $baremetal->configurationItem->scomp_id, $baremetal->id);
        ...
        $account = Account::byScompId($managedAccountScompId)->firstOrFail();
        $region = Region::upsert(
            $row[static::REGION_SCOMP_ID],
            [
                'name' => $row[static::REGION_SCOMP_ID],
                'managed_service_provider_id' => $account->managed_service_provider_id
            ]);
        $this->compositeKeyContainer->set('region', $account->managed_service_provider_id . ":" . $region->configurationItem->scomp_id, $region->id);

        $az = AvailabilityZone::upsert(
            $row[static::AZ_SCOMP_ID],
            [
                'name' => $row[static::AZ_SCOMP_ID],
                'region_id' => $region->id
            ]);
        $this->compositeKeyContainer->set('availability_zone', $account->managed_service_provider_id . ":" . $region->configurationItem->scomp_id . ":" . $az->configurationItem->scomp_id, $az->id);
