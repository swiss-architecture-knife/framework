...
class ScopeTemplatesSheet extends AbstractSwarkExcelSheet implements FromGenerator, WithTitle
{
    ...
    {
        yield ['name', 'scomp-id', 'description', 'Swark\DataModel\Infrastructure\Repository\Scope\ItemsByScompId', '{"key":"value"}', ''];
    }
    ...
    protected function importRow(RowContext $row)
    {
        ...
        $scopeTemplate = Template::upsert($row->nonEmpty(Column::SCOMP_ID_COLUMN),
            [
        ...
        $this->compositeKeyContainer->set('scope_template', $scopeTemplate->configurationItem->scomp_id, $scopeTemplate->id);
    }
}
