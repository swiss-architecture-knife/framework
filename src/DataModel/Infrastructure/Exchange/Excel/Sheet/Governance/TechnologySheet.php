...
class TechnologySheet extends AbstractSwarkExcelSheet implements FromGenerator, WithTitle
{
    ...
    {
        $technology = Technology::upsert($row->nonEmpty(Column::SCOMP_ID_COLUMN),
            [
        ...
        $this->compositeKeyContainer->set('technology', key: $technology->configurationItem->scomp_id, value: $technology->id);
        $latest = $technology->latest();
        $this->compositeKeyContainer->set('technology_version', key: $technology->configurationItem->scomp_id . ":latest", value: $latest->id);
        ...
        if (!empty($versions)) {
            ...
            foreach ($versions as $versionString) {
                ...
                $version = $technology->versions()->updateOrCreate(['name' => $versionString]);
                $this->compositeKeyContainer->set('technology_version', key: $version->configurationItem->scomp_id, value: $version->id);
            }
        }
    }
